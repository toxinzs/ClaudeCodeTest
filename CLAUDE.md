...
- **Updated character customization/cosmetics.** Also explicitly flagged as a later item, not scoped yet. `charCreate.js`/`AVATAR_OPTIONS` is now ported to Phaser (a fixed emoji-avatar picker), and the overworld player glyph currently gets a squash/stretch tween as a placeholder walk-cycle (added in Phase 6, alongside continuous movement) — still not real sprite art. This roadmap item is about going further than the current emoji pick once it's revisited: a real character sprite (replacing the "just a head" emoji) with actual walk-cycle frames, likely unlockable/purchasable cosmetics later. No design decisions made yet (what's customizable, whether cosmetics are earned/bought, whether this ties into the Mart/badge-tier item system). Note it alongside Mega Evolution as "later, like everything else" rather than building ad hoc.
...
- Walkable tile-grid maps (Home, Town, Wild Zone Trail, League Map) with real sprite art (PokeAPI official-artwork hotlinked) and a real font pairing (Silkscreen/Nunito). Continuous held-key movement (not one tile per keypress) and a zoomed follow-camera on outdoor maps (Town/Trail/League) so they read as a place you walk through rather than a fully-visible game board.
