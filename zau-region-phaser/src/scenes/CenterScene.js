...
export default class CenterScene extends Phaser.Scene {
  ...
  renderPrompt() {
    ...
    this.add.text(GAME_W / 2, 60, needsHealing
      ...
      { fontFamily: 'Nunito, sans-serif', fontSize: '13px', color: '#c8c8d8', wordWrap: { width: GAME_W - 60 }, align: 'center' }
    ).setOrigin(0.5, 0);

    if (needsHealing) {
      const bg = this.add.rectangle(GAME_W / 2, 130, 200, 34, 0x232640).setStrokeStyle(1, 0x3a3d5c).setInteractive({ useHandCursor: true });
      this.add.text(GAME_W / 2, 130, 'Heal my team', { fontFamily: 'Nunito, sans-serif', fontSize: '13px', color: '#e8e8f0' }).setOrigin(0.5);
