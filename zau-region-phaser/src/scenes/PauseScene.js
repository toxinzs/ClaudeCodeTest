...
export default class PauseScene extends Phaser.Scene {
  ...
  create() {
    ...
    items.forEach(([label, fn], i) => {
      ...
      this.add.text(GAME_W / 2, y, label, { fontFamily: 'Nunito, sans-serif', fontSize: '13px', color: '#e8e8f0' }).setOrigin(0.5);
      bg.on('pointerdown', fn);
    });
    this.saved = this.add.text(GAME_W / 2, 92 + items.length * 44, '', { fontFamily: 'Nunito, sans-serif', fontSize: '12px', color: '#7fc47f' }).setOrigin(0.5);
