  ...
  create() {
    ...
    if (!entries.length) {
      this.add.text(GAME_W / 2, GAME_H / 2, 'No quests yet. Talk to people — Zau is full of favours to do.', {
        fontFamily: 'sans-serif', fontSize: '13px', color: '#8a8aa0', wordWrap: { width: GAME_W - 80 }, align: 'center'
      }).setOrigin(0.5);
      return;
    }
    let y = 60;
    for (const [key, q] of entries) {
      ...
      this.add.text(36, y, `${done ? '✅' : '📜'} ${q.name}`, { fontFamily: 'sans-serif', fontSize: '14px', color: done ? '#8a8aa0' : '#e8e8f0' });
      this.add.text(36, y + 20, `${q.giver} · ${q.district}`, { fontFamily: 'sans-serif', fontSize: '11px', color: '#6a6a80' });
      this.add.text(36, y + 36, q.summary, { fontFamily: 'sans-serif', fontSize: '11px', color: '#8a8aa0', wordWrap: { width: GAME_W - 72 } });
      const objY = y + 36 + 34;
      this.add.text(36, objY, done ? `Reward: ${q.reward}` : `▶ ${nextObjective(key)}`, { fontFamily: 'sans-serif', fontSize: '12px', color: done ? '#7fc47f' : '#e8d27a', wordWrap: { width: GAME_W - 72 } });
      y = objY + 40;
    }
  }
}
