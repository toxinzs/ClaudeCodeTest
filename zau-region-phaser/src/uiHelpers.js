...
export function drawModalBackdrop(scene, title) {
  ...
  scene.add.text(GAME_W / 2, 20, title, {
    fontFamily: 'Nunito, sans-serif', fontSize: '15px', color: '#e8e8f0', wordWrap: { width: GAME_W - 80 }, align: 'center'
  }).setOrigin(0.5, 0);
}

export function addCloseButton(scene, onClose) {
  const btn = scene.add.text(GAME_W - 24, 18, '✕', { fontFamily: 'Nunito, sans-serif', fontSize: '16px', color: '#c8c8d8' })
...
export function addRow(scene, y, opts) {
  ...
  scene.add.text(64, y - 8, opts.title, {
    fontFamily: 'Nunito, sans-serif', fontSize: '13px', color: opts.dim ? '#6a6a7a' : '#e8e8f0'
  }).setOrigin(0, 0);
  if (opts.subtitle) {
    scene.add.text(64, y + 8, opts.subtitle, { fontFamily: 'Nunito, sans-serif', fontSize: '11px', color: '#8a8aa0' }).setOrigin(0, 0);
  }
  if (opts.buttonLabel) {
    const bw = 56, bh = 24;
    const bx = GAME_W - 24 - bw / 2 - 12, by = y;
    const bg = scene.add.rectangle(bx, by, bw, bh, 0x232640).setStrokeStyle(1, 0x3a3d5c);
    const label = scene.add.text(bx, by, opts.buttonLabel, { fontFamily: 'Nunito, sans-serif', fontSize: '11px', color: '#e8e8f0' }).setOrigin(0.5);
    if (opts.onButton) {
...
export function addActionBar(scene, buttons, y = GAME_H - 20) {
  ...
  buttons.forEach((b, i) => {
    ...
    const label = scene.add.text(x, y, b.label, { fontFamily: 'Nunito, sans-serif', fontSize: '11px', color: '#e8e8f0' }).setOrigin(0.5);
